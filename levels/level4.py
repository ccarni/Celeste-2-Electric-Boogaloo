...
class Level4(Level):
    ...
    def draw_mountain(self, width, height):
        ...
        surf.fill((0, 0, 0, 0))
        pygame.draw.polygon(surf, (250, 255, 250),
                            [(0, surf.get_height()), (surf.get_width(), surf.get_height()), (surf.get_width() / 2, 0)])
        return surf
    ...
    def draw_moon(self):
        ...
        for i in range(moon.get_width()):
            for j in range(moon.get_width()):
                amount = [random.randint(-10,
                                         10)] * 3  # This duplicates the number three times to ensure we don't get colors
